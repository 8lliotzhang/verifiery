...
MAX_CHARS = 10000
global charlesThinksItsAi 
#do this!!
instructions = (
    "Intent:"
    "You are to determine if the provided text has been written by humans or generated by an AI model. Consider the following."
    
    "Instructions:"

    "Text which is too short should be rejected."    
    #hmm or do i just look at chars...
    "Language: AI-generated text often uses larger complex words while having a simpler meaning."
    
    "Structure: AI has a smooth flow in text, with an overly polished, predictable pattern of explanation and evidence. Humans tend to vary sentence structure more, and use more complex structures."    
    ...
    "Consider the voice of the text - if it feels stiff or overly formal feel, it may be AI-generated."
    "Imperfections - slight grammatical imperfections may be signs of human editing."
    "Ideas: AI generated models tend to have bland ideas or repeat them throughout the text."
    
    "Consider how consistent the provided text is with the output of an AI language model."
    
    "Word Choice: AI may use large, long or complicated words, while human writers (especially at the high school level) will tend to prefer shorter words."
    
    "Please generate your thought process or an internal monologue as you determine whether the provided text is AI-generated or human."
    "You are to Weigh and Consider the reasons the text might be AI-generated, and the reasons it may be human written."
    "Understand that Human-written text may reflect consistency in academic writing, especially for academic assignments, which might not showcase high variation in words. if focusing on clarity and coherence."
    ...
    "Outputs. Provide a verdict. your options are (Very Likely Human, Likely Human, Inconclusive, Likely AI, Very Likely AI). Output STRICTLY in the following format, maintaining the round brackets: 'response:([summary of thoughts, plaintext overview])\nverdict:([verdict])\nscore:([AI confidence score: 2 place decimal between 0 and 1]). ensure that the round brackets are correctly placed, especially between the score decimal value. It must be perfectly formatted every time."
    "Compare text to the provided samples."
...
AiSamples = (
    ...
    "These essays have AI-generated characteristics such as "
)

#----------------------------#
#    Obscurity Initialize    #
#----------------------------#
...
def main(userEssayIn):
    output_label.config(text="if ur seeing this, something may have gone wrong :( oops")

    try:
        total_score = 0
        runs = 3  # Number of repetitions - 5 for being sure, 3 for testing
        successRuns = 0
...
def process_input():
    ...
    #handling avg
    #placeholder for testing main2
    #avg_score = 0.5
    
    #actually run main here...
    avg_score = main(user_input)
    avg_score_percent = avg_score * 100
    print("AVG SCORE IS:" +str(avg_score))
    
    #obs
    obs_score = main2(user_input)
    print("OBS PERCENTAGE CONF IS: "+ str(obs_score))
    print("OBS THINKS IS AI PRESENT: " + str(charlesThinksItsAi))
    if charlesThinksItsAi is True:
        final_obsscore = (50 + obs_score/2)

    else:
        final_obsscore = (50 - obs_score/2)

    print ("FINAL OBSSCORE IS: "+ str(final_obsscore))
    final_score = (((avg_score_percent*0.8) + (final_obsscore*0.2)))
    
    ##############
    #    STOP    #
    ##############    #After this point NO FURTHER PROCESSING happens to final_score, use scorestr and then final_scorestr
    
    
    scorestr = str(final_score)
    print("FINAL FINAL SCORE IS: " + scorestr)
    
    length = 5
    # num of chars in truncated string.

    #final_scorestr = scorestr.substring(0,length)
    
    if avg_score is not None:
        output_label.config(text=f"Our Confidence it's AI: {scorestr}%")
